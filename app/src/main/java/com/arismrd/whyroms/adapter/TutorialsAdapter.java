...
public class TutorialsAdapter extends RecyclerView.Adapter<TutorialsAdapter.ViewHolder> {
    private List<ModelTutorials> allVideos;
    private Context context;

    public TutorialsAdapter(Context ctx, List<ModelTutorials> videos){
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_tutorial,parent,false);
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, final int position) {
        ...
        Picasso.with(context).load(allVideos.get(position).getImageUrl()).into(holder.videoImage);

        holder.vv.setOnClickListener(v -> {
            Bundle b = new Bundle();
            b.putSerializable("videoData",allVideos.get(position));
            Intent i = new Intent(context, PlayTutorialActivity.class);
            i.putExtras(b);
